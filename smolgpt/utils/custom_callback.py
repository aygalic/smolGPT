...
class AutoLoadLastCheckpointCallback(Callback):
    """Helper to automatically load last model checkpoint before inference

    Parameters
    ----------
    tokenizer_type : str, optional
        Type of tokenizer used with the given architecture, by default None
    """
    ...
    def on_predict_start(self, trainer, pl_module):
        if not trainer.ckpt_path:  # If no checkpoint was explicitly provided
            ...
            if self.tokenizer_type is not None:
                checkpoint_files = [
                    f for f in checkpoint_files if self.tokenizer_type in f
                ]
            if checkpoint_files:
